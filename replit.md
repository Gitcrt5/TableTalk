...
- July 16, 2025. **1PASSWORD AUTHENTICATION OPTIMIZATION** - Reduced frequent `/api/user` requests that were causing 1Password to prompt for password saving on every page transition. Implemented 5-minute caching for authentication queries, disabled automatic refetching on component mount, and added cache headers to `/api/user` endpoint to prevent password manager interference.
