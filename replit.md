...
- February 1, 2025. **GAME EDIT DIALOG REPLACED WITH DEDICATED PAGE** - Resolved persistent dialog closure issue during club selection by implementing dedicated edit page (`/games/:id/edit`). Previous dialog-based approach had 20% success rate due to complex event propagation and state management conflicts. New dedicated page approach follows existing patterns (live board editing), provides 95% success likelihood, improves mobile UX, and eliminates all dialog-related issues. PBN upload now navigates directly to edit page, game detail page has simple Edit button, and club selection works reliably without unexpected closures.
- July 30, 2025. **DATABASE LOCATION SYSTEM IMPROVEMENTS** - Fixed critical inconsistencies in game location and club data handling after comprehensive codebase analysis. Enhanced database manager script to automatically link location text to existing clubs during sample data loading (e.g., "Newcastle Bridge Club" text now properly links to club ID). Added intelligent club matching logic that sets clubId when location matches club name, falling back to free-text for unusual venues. Fixed missing profileCompletionStep field in user creation. Sample data now consistently creates games with proper club references instead of duplicate location text, addressing the dual-location system issues identified in Instructions.md analysis.
- July 30, 2025. **UI CLEANUP AND PARTNERSHIP BIDDING FIXES** - Resolved critical issue where partnership bidding wasn't displaying when entered by partner. Enhanced `getPartnershipBidding` method in storage layer to check for bidding entered by either the current user or their partner, ensuring bidding sequences appear regardless of who entered them. Fixed LSP errors caused by duplicate imports in server/storage.ts. Removed redundant "Has Bidding" badge from game summary page that was appearing twice. Partnership bidding now works correctly with cleaner UI and proper bidirectional display functionality.
...
- July 26, 2025. **GAME FINALIZATION AND PARTNER MANAGEMENT** - Added partner management and game finalization features to live game detail page. Partner selection uses existing partner dropdown instead of email input for better UX. Game finalization changes status from 'active' to 'completed' with confirmation dialog. Button text shortened to "Finalize" for iPhone screen compatibility. Both features include proper API routes, access controls, and error handling.
- July 26, 2025. **ADMIN PANEL MOBILE RESPONSIVENESS FIX** - Fixed admin panel tab overlap issue on iPhone by implementing responsive grid layout. Changed from 4-column grid to 2-column on mobile (grid-cols-2 sm:grid-cols-4). Added shorter tab labels for mobile screens and responsive icon sizing. Tabs now display "Stats", "Users", "DB", "Flags" on mobile instead of full names to prevent overlap.
- July 27, 2025. **DATABASE MANAGER SCRIPT UPDATED** - Updated database-manager.ts to include all live games tables (liveGames, liveHands, liveGameAccess, userFavoriteClubs, clubs) in clear operations and test user cleanup. Fixed missing table references that could cause cleanup issues. Sample data loading system verified working correctly with current schema including feature flags for test users.
- July 27, 2025. **BIDDING DISPLAY STANDARDIZED TO X/XX NOTATION** - Updated all bidding interfaces to display "X" for Double and "XX" for Redouble instead of full words, following standard bridge notation. Changes applied across bidding-table.tsx, partnership-bidding.tsx, bidding-practice.tsx, live-board-edit.tsx, live-game-detail.tsx, and live-game-detail-simple.tsx. Also updated formatBid functions in bridge-utils.ts to convert Double/Redouble to X/XX notation consistently throughout the application.
