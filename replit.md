...
- July 30, 2025. **PARTNERSHIP BIDDING DISPLAY FIXED** - Resolved critical issue where partnership bidding wasn't displaying when entered by partner. Enhanced `getPartnershipBidding` method in storage layer to check for bidding entered by either the current user or their partner, ensuring bidding sequences appear regardless of who entered them. Fixed LSP errors caused by duplicate imports in server/storage.ts. Partnership bidding now works correctly with proper bidirectional display functionality.
